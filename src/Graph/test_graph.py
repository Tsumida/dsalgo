...
class TestGraph(TestCase):
    ...
    def test_usage(self):
        ...
        _ = Graph.from_edge_list(PHY_ADJ_LIS, lis)

        _ = Graph().set_type(PHY_MAT).from_list(lis)
        _ = Graph.from_edge_list(PHY_MAT, lis)

    def test_graph_storage(self):
        lis = CASES["tree"]
        g = Graph.from_edge_list(PHY_ADJ_LIS, lis)
        sto = g.get_storage()
        assert isinstance(sto, WeightedAdjLis)

        g2 = Graph.from_edge_list(PHY_MAT, lis)
        sto2 = g2.get_storage()
        assert isinstance(sto2, AdjMat)

    def test_city(self):
        g1 = Graph.from_edge_list(PHY_MAT, Citys)
        g2 = Graph.from_edge_list(PHY_ADJ_LIS, Citys)
...
class TestAdjMat(TestCase):
    ...
    def test_delete_node(self):
        g1 = AdjMat.from_edge_list(CASES["circle"])
        assert g1.get_num_node() == 5
        assert g1.get_num_edge() == 5
    ...
    def test_delete_edge(self):
        ...
        :return:
        """
        g = AdjMat.from_edge_list(CASES["4-complete"])
        assert g.del_edge(1, 2) == (1, 2, 1)
        assert g.del_edge(2, 1) == (2, 1, 1)
        assert g.get_num_edge() == 10, f"error: {g.get_num_edge()}"
        assert g.get_num_node() == 4
