...
# leetcode 310
from testFunctions import timer
...
class Solution:
    ...
    def findMinHeightTrees(self, n: int, edges: list) -> list:
        ...
        # for n = 909, cost 0.46s
        def dfs_get_height(index:int) -> int:

            res = 0     # height of this root.
            visit[index] = self.GRAY
            for neighbour in adj_list[index]:
                if visit[neighbour] == self.WHITE :
                    res = max(dfs_get_height(neighbour)+1, res)

            #print(f"{index} visited.", end=' ')
            visit[index] = self.BLACK # visited
            # res == 0 <=> index is a leaf.
            return res

        # for n = 909, cost 4.1s
        def get_height(index:int, adj:list, visited:list) -> int:
            res = 0
            Q = Queue()
            Q.put((index, res))
            while not Q.empty():
                node, height = Q.get()
                visited[node] = self.GRAY
                #print(node, height)
                for neigh in adj[node]:
                    if visited[neigh] == self.WHITE:
                        Q.put((neigh, height+1))
                visited[node] = self.BLACK
                res = max(res, height)
            return res


        visit = [self.WHITE for x in range(n)]
        ...
        for index in range(n):
            height = dfs_get_height(index)
            #height = get_height(index, adj_list, visit)
            #print("\nindex = {}, height = {}".format(index, height))
            result.append((index, height))
            min_height = min(min_height, height)
            visit = [self.WHITE for x in range(n)]  # reset.

        # now we got max_height
        #print("miao ------ min_height = {}".format(min_height))
        return [i for i, h in result if h <= min_height]
    '''
    def findMinHeightTrees(self, n: int, edges:list) -> list:
        def dfs(index:int ):
            pass
        ...
        while len(adj) > 2:
            ...
            for u, neighbours in adj.items():
                if len(neighbours) == 1:
                    count_leaves += 1
                    q.put((u, neighbours.pop()))    # 只有一个，所以pop是随机的
                    neighbours.clear()
            # simulating skin the onion.
            while not q.empty():
                v, u = q.get()
                adj[u].remove(v)
                # 还要去掉叶子节点！
        ...
        #print("leaves count = %d " % count_leaves)

        for u, _ in adj.items():
            res.append(u)

        return res


    def findMinHeightTrees_Fastest(self, n, edges):
            """
            :type n: int
            ...
            if n == 1:
                return [0]
            # 用set()代替了list()
            allNodes = [set() for i in range(n)]

            for edge in edges:#每个节点的邻接点
                allNodes[edge[0]].add(edge[1])
                allNodes[edge[1]].add(edge[0])

            # 找出所有叶子
            leaves = [i for i, nodeEdges in enumerate(allNodes) if len(nodeEdges) == 1]
            ...
            while n > 2:
                ...
                for leaf in leaves:
                    ...
                    # pop()随机返回集合中一个元素，但是这里只有一个元素
                    adj = allNodes[leaf].pop()
                    allNodes[adj].remove(leaf)  # 删除叶子

                    if len(allNodes[adj]) == 1:
        ...
        #visit.fromkeys(range(n), [self.WHITE])
